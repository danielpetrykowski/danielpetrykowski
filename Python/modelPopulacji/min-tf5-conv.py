...
daneZaraza_1 = tf.concat([w_zero, daneDrzewa, w_zero], 0)
...
daneZaraza_sqare = tf.concat(daneZaraza_transform, 1)
...
x_daneDrzewa=tf.reshape(daneZarazy, [-1,10,10,1])
...
with tf.Session() as sess:
    ...
    if a==2:
        saver.restore(sess, "/tmp/modelConv3.ckpt")
    
    for i in range(14719,75000):
...
#        while robaki.liczbaWszystkichKornikow() > 0:
    ...
    #        if sprawdzam % 50 == 0:
            ...
            plt.figure(3)
            plt.imshow(outR, interpolation='nearest')
            plt.grid(True)
            plt.show()
        ...
        if sprawdzam % 2000 == 0:
            save_path = saver.save(sess, '/tmp/modelConv3.ckpt')
            print("Model saved in file: %s" % save_path)
            plik = open('iteracjeConv3.txt', 'w')
